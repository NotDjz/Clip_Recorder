...
```
py tests\test_invariants.py     # 22 guard rails: the invariants this project already paid for
...
**There is no linter or type checker in this repo.** The syntax check is `py -m py_compile src\clip_recorder.pyw` — worth running after editing, because a `.pyw` is not exercised by any casual run and a syntax error otherwise surfaces only when the app silently fails to launch (windowed, no console).

**Run `mutation_check.py` after touching `test_invariants.py`.** A guard rail that cannot fail is worse than none — it buys false confidence. That checker caught two tests of mine that were silently useless: one matched on filenames that snapshotting had already renamed, and one only ever exercised the no-audio branch because its fake never wrote the WAV to disk. It currently breaks 10 invariants on purpose and all 10 are caught by their named test.
