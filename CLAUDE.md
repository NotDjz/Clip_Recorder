...
```
py tests\test_invariants.py     # 28 guard rails: the invariants this project already paid for
...
- **The save chime is synthesised, never a shipped file.** Two sines (220 Hz + 330 Hz, 200 ms, 4 ms attack then exponential decay — without the attack a sine starts at full amplitude and clicks), rendered once into WAV bytes and cached. It replaced `SystemExclamation`, which is also what Windows plays for error dialogs. **`winsound` refuses `SND_ASYNC` together with `SND_MEMORY`** (`RuntimeError: Cannot play asynchronously from memory`), so the play is synchronous and gets its own daemon thread — called inline it would sit in front of the notification banner for the length of the sound. Measured: 9 ms to synthesise once, 19 KB, and `play_save_sound()` returns in 1 ms.
- FFmpeg process: `CREATE_NO_WINDOW` flag, graceful kill via `stdin.write(b"q")` → wait → terminate → kill
- Notification banner: `WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE` + `WDA_EXCLUDEFROMCAPTURE`, applied while the window is still `withdraw()`n (before first `deiconify()`) — all four pinned by `test_banner_keeps_its_window_styles`.
  - **You cannot screenshot the banner.** `WDA_EXCLUDEFROMCAPTURE` hides it from every screen-capture API, `PIL.ImageGrab` included — an empty screenshot is the feature working, not a broken banner. To photograph the design, monkeypatch `SetWindowDisplayAffinity` to a no-op first; the blank capture with it in place is itself the proof it stays out of the clip.
  - The banner is a **rounded pill**, not a rectangle: `-transparentcolor` punches out a key colour on an `overrideredirect` window, so a `tk.Canvas` can draw two circles plus a rectangle and the corners are genuinely round. Verified to coexist with the three `WS_EX` flags above. It takes its colours from `BG`/`FG`/`FG2`/`ACCENT` — it used to be the only place in the product using `#1a1a2e` and `#22cc66`, which belong to no palette here. It shows the clip's length, the one fact that confirms the buffer setting took effect, and no tick mark (that repeats "saved" in a second modality).
  - Without `WS_EX_NOACTIVATE`, showing the banner can steal foreground activation from windowed/borderless games (exclusive-fullscreen games are protected by Windows from this; windowed ones are not).
